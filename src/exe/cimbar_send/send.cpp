...
template <typename TP> TP wait_for_frame_time(unsigned delay, const TP &start) {
...
int main(int argc, char **argv) {
  cxxopts::Options options("cimbar video encoder",
                           "Draw a bunch of weird static on the screen!");
  ...
  options.add_options()("i,in", "Source file",
                        cxxopts::value<vector<string>>())(
      "c,colorbits", "Color bits. [0-3]",
      cxxopts::value<unsigned>()->default_value(turbo::str::str(colorBits)))(
      "e,ecc", "ECC level",
      cxxopts::value<unsigned>()->default_value(turbo::str::str(ecc)))(
      "f,fps", "Target FPS",
      cxxopts::value<unsigned>()->default_value(turbo::str::str(defaultFps)))(
      "m,mode",
      "Select a cimbar mode. B is new to 0.6.x. 4C is the 0.5.x config. [B,4C]",
      cxxopts::value<string>()->default_value("4C"))(
      "s,static", "Disable shaking",
      cxxopts::value<bool>()->default_value("false"))(
      "z,compression", "Compression level. 0 == no compression.",
  ...
  colorBits = std::min(3u, result["colorbits"].as<unsigned>()); // [0,3]
  compressionLevel = result["compression"].as<int>();
  ecc = result["ecc"].as<unsigned>();
  shaking = !result["static"].as<bool>();
  ...
  configure(colorBits, ecc, compressionLevel, shaking, legacy_mode);
  ...
  while (true)
    for (unsigned i = 0; i < infiles.size(); ++i) {
      ...
      // TODO: maybe delay is the wrong thing to do here. Might be best to just
      // kick out any files that fail to read? we can then error out properly if
      // all inputs are bad, which would be nice.
      {
        ...
        // start encode_id is 109. This is mostly unimportant (it only needs to
        // wrap between [0,127]), but useful for the decoder -- because it gives
        // it a better distribution of colors in the first frame header it sees.
        if (!encode(reinterpret_cast<unsigned char *>(contents.data()),
                    contents.size(), static_cast<int>(i + 109))) {
          std::cerr << "failed to encode file " << infiles[i] << std::endl;
          continue;
        }
      }

      // after loading our current file, render frames to the screen until
      // next_frame() loops
      int frameCount = 0;
      do {
        start = wait_for_frame_time(delay, start);
        if (render() < 0)
          return 0;
      } while (++frameCount == next_frame()); // when next_frame() finally
                                              // loops, we roll to the next file
    }
