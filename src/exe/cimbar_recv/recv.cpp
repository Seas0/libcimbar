...
void mouse_button_callback_showing(GLFWwindow *window, int button, int action,
                                   int mods) {
...
int main(int argc, char **argv) {
  cxxopts::Options options("cimbar video decoder",
                           "Use the camera to decode data!");
  ...
  fountain_decoder_sink<cimbar::zstd_decompressor<std::ofstream>> sink(
      outpath, chunkSize);
  ...
  while (true) {
    ...
    // only if windows is not minimized
    if (showing)
      window.show(mat, 0);
